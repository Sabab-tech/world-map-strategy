...
function loadGameCities() {
    fetch('cities.json?v=' + new Date().getTime())
        .then(res => {
            if (!res.ok) throw new Error("Local cities.json load failed");
            return res.json();
        })
        .then(data => {
            if (data.regions) {
        ...
        .catch(err => console.log("Waiting for cities data syncing...", err));
}
...
fetch('countries.json?v=' + new Date().getTime())
    .then(res => {
        if (!res.ok) throw new Error("Local countries.json load failed");
        return res.json();
    })
    .then(countryConfig => {
    ...
    .catch(err => console.log("Error running map engine:", err));
